...
object Room {
    ...
    /**
     * Какая точка холста по горизонтали встаёт в середину экрана.
     *
     * Холст заполняет экран по высоте, и на телефонах от его ширины видно
     * 0.75 (20:9) – 0.77 (1080 × 2340), остальное срезается. Кадр тянут две
     * стороны: слева торшер, который понадобится для света, справа питомец,
     * стоящий на оси комнаты ([ROOM_AXIS_X]). По центру холста под нож шла бы
     * треть торшера, при 0.418 торшер помещался целиком, но питомец упирался
     * в правый край экрана.
     *
     * 0.436 — край, на котором на 20:9 ещё целиком видны и ванна (левый борт
     * 0.061), и питомец (правый край силуэта 0.811). От торшера срезается
     * только кончик кольца «Сатурна»: шар абажура и подставка начинаются от 0.07.
     */
    const val FOCUS_X = 0.436f

    /**
     * Ось комнаты: середина пола в перспективе.
     *
     * Найдена по доскам, а не по холсту: доски сходятся к одной точке, и та,
     * что идёт строго вертикально, стоит на 0.508 на всех строках пола. Питомец
     * в зале встаёт на эту ось — тогда центральная доска уходит вглубь ровно
     * у него из-под ног, и он стоит посреди комнаты. По середине экрана он
     * читался сдвинутым к торшеру: доски сходились правее него.
     */
