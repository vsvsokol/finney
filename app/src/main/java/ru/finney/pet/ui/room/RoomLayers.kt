...
object Room {
     ...
     *
     * Холст заполняет экран по высоте, и на телефоне 1080 × 2340 от его ширины
     * видно 0.77 — остальное срезается. По центру (0.5) под нож шла бы треть
     * торшера, а он понадобится для света. При 0.418 левый край кадра
     * приходится ровно на торшер (0.0326), а срезается правый край занавески
     * и 17% вида из окна — декорация, за которую ничего не держится.
     */
    const val FOCUS_X = 0.418f
    ...
    private fun petCentre(spot: RoomSpot): Float = when (spot) {
        // Посередине экрана, а не холста: холст срезан по бокам несимметрично.
        RoomSpot.LIVING -> FOCUS_X
