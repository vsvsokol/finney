...
Главный экран → меню (☰) → **«Мини-игры»**. Там шесть карточек с картинками. Нажатие
...
**`goal_race` — Дорога к цели.** `goal` — `{ "label", "price", "emoji" }`. `days` — сколько
...
**Осторожно с минимумом ТЗ п. 2.6: не меньше 6 заданий по 3 темам.** Сейчас их ровно шесть,
и тест «минимальный объём контента» покраснеет, если убрать хоть одну. Убирая игру, добавьте
вместо неё другую — например, второй «Конвейер» на другие вещи (ниже).
...
Например, второй «Конвейер» про школьные вещи:

1. Скопировать блок `game_sorter` целиком, поставить после него через запятую.
2. Поменять `id` на новый (`game_sorter_school`), `title`, `intro`, объяснения и вещи.
...
- **Проверено не до конца.** Код собран, 80 unit-тестов зелёные. Перед демонстрацией пройти
  все шесть игр на физическом устройстве (ТЗ п. 3.1) — и выигрыш, и проигрыш.
- **Эмодзи вместо рисунков** у мячика, наклеек, куртки, щётки, самоката, зонта, лимона.
  Нужны рисунки в стиле кита — `design/exports/items/item_*.png`, потом `pack_items.py`.
